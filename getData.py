...
activityColumns = ['name', 'id' , 'upload_id', 'type', 'distance', 'moving_time',   
         'average_speed', 'max_speed','total_elevation_gain', 'average_cadence', 'average_watts', 'average_heartrate',
         'start_date_local' , 'achievement_count', 'kudos_count', 'comment_count','athlete_count'
       ]
...
def setSplits(ActivityId, header):
    splitsUrl = activitiesUrl + '/' + str(ActivityId)
    print(f'Getting split information for [{ActivityId}]')
    ...
    while True:
        try:
            ...
            for currentActivity, _ in enumerate(activityRequest):
                WhatWeWant = ['Run','Ride','VirtualRide','EBikeRide']
                Type = activityRequest[currentActivity]['type']
                ActivityId = activityRequest[currentActivity]['id']
                print(f'Found activity id {ActivityId}, activity type {Type}')
                if Type in WhatWeWant :
                    # Add columns that must exist
                    activities.loc[index,'id'] = ActivityId
                    ...
                    if 'average_watts' in activityRequest[currentActivity]:
                        activities.loc[index,'average_watts'] = activityRequest[currentActivity]['average_watts']
                    else:
                        activities.loc[index,'average_watts'] = 0
                    if 'average_heartrate' in activityRequest[currentActivity]:
                        activities.loc[index,'average_heartrate'] = activityRequest[currentActivity]['average_heartrate']
                    else:
                        activities.loc[index,'average_heartrate'] = 0
                    index += 1
                    splitsDataSet = setSplits(ActivityId, header)
                    # Add to split dataset
                    splits = pandas.concat([splits, splitsDataSet])
                else:
                    print(f'activity type {Type} is not processed right now')
